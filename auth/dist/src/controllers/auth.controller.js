...
import { isEmailValid, isServiceMethodSuccess, isPasswordValid, hashPassword, isPasswordMatch, createSessionId, generateRefreshToken, generateAccessToken, verifyTwoFactorToken, generateQRCode, generateTwoFactorSecret, isGenerateQRCodeSuccess } from '../utils';
export const handleUserRegister = async ({ body, set, jwt, redis })=>{
    ...
    if (!isServiceMethodSuccess(getTotalUsers)) {
...
export const handleUserLogin = async ({ jwt, body, set, redis })=>{
...
export const handleTokenRefresh = async ({ set, jwt, redis, body, request })=>{
...
export const handleUserLogout = async ({ body, set, request, jwt, redis })=>{
    const sessionId = request.headers.get('X-Session-Id');
    const refreshToken = request.headers.get('X-Refresh-Token');
...
export const handleEnableTwoFactorAuth = async ({ body, set, request, jwt, redis })=>{
    const sessionId = request.headers.get('X-Session-Id');
    const refreshToken = request.headers.get('X-Refresh-Token');
    const accessToken = request.headers.get('Authorization')?.split(' ')[1] ?? '';
    const { userId } = body;
    ...
    if (!isServiceMethodSuccess(userEnabledTwoFactor)) {
...
export const handleDisableTwoFactorAuth = async ({ body, set, request, jwt, redis })=>{
    const sessionId = request.headers.get('X-Session-Id');
    const refreshToken = request.headers.get('X-Refresh-Token');
    const accessToken = request.headers.get('Authorization')?.split(' ')[1] ?? '';
    const { userId } = body;
    ...
    if (!isServiceMethodSuccess(userDisabledTwoFactor)) {
